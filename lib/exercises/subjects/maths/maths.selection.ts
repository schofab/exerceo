...
function computeBaseScore(
  ...
  if (weaknesses.includes('mathématiques') || weaknesses.includes('maths')) {
    if (exercise.generalLevel === 'beginner') score += 2;
  }

  if (strengths.includes('mathématiques') || strengths.includes('maths')) {
...
function resolveHistoryContext(history: ExerciseSelectionHistory): {
  ...
  if (history.recentSubskills) {
    ...
    for (const ex of mathsExercises) {
      if (idSet.has(ex.id)) {
...
/**
 * Sélectionne des exercices de mathématiques adaptés au profil de l'élève.
 *
 ...
 */
export function getMathExercises(params: {
  profile: StudentProfile;
  skill?: MathSkill;
  limit?: number;
  history?: ExerciseSelectionHistory;
  ...
  // 1. Filtrage de base
  const pool = mathsExercises.filter(
    (ex) =>
      ex.subject === 'mathematiques' &&
