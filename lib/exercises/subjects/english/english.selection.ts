...
function computeBaseScore(
  ...
  if (weaknesses.includes('anglais') || weaknesses.includes('english')) {
    if (exercise.generalLevel === 'beginner') score += 2;
  }

  if (strengths.includes('anglais') || strengths.includes('english')) {
...
function resolveHistoryContext(history: ExerciseSelectionHistory): {
  ...
  if (history.recentSubskills) {
    ...
    for (const ex of englishExercises) {
      if (idSet.has(ex.id)) {
...
function selectWithDiversity(
  scored: Array<{ exercise: EnglishExercise; score: number }>,
  limit: number
): EnglishExercise[] {
  const selected: EnglishExercise[] = [];
...
export function getEnglishExercises(params: {
  profile: StudentProfile;
  skill?: EnglishSkill;
  limit?: number;
  history?: ExerciseSelectionHistory;
  ...
  const pool = englishExercises.filter(
    (ex) =>
      ex.subject === 'anglais' &&
